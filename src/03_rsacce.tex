...
\begin{definition}[Channel Binding] \label{def:rsacce-cb}
 ...
 \begin{itemize}
  ...
  that has a matching conversation for forward secure key with $\pi^s_{j,0}$.}

  \item{For all $0\leq k \leq \ell$, the adversary does not issue a $\Reveal$ query
  to $\pi^c_{i,k}$ such that $\pi^c_{i,k}$ has a matching conversation with $\pi^s_{j,k}$.}
...
forward secure key only with the client
